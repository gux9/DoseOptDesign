...
calc_sample_size_rose_approx <- function(pL, delta,
  ...
  # Nominal PCS (normal approximation): equals the targets by construction.
  pcs_L <- pnorm((lambda_u - delta_mu_L) / sqrt(v_L / n))
  pcs_H <- 1 - pnorm((lambda_u - delta_mu_H) / sqrt(v_H / n))
  
  # Attained PCS (exact). Under ROSE the utility is the response indicator,
  # so the outcome vector is (p, 0, 1 - p, 0) with u = (1, 1, 0, 0).
  attained <- calc_attained_pcs(
    pi_L_SL = c(pL_L, 0, 1 - pL_L, 0), pi_H_SL = c(pH_L, 0, 1 - pH_L, 0),
    pi_L_SH = c(pL_H, 0, 1 - pL_H, 0), pi_H_SH = c(pH_H, 0, 1 - pH_H, 0),
    u = c(1, 1, 0, 0), n = n, lambda_u = lambda_u, den = 1)
