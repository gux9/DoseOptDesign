...
#' @param den Denominator for the integer lattice used to compute the attained
#'          PCS (default 10). Must satisfy that `u * den` is integral.
#'
#' @return List containing:
#'   - n: Sample size per arm (integer)
#'   - lambda_u: Decision threshold
#'   - PCS_L: Nominal PCS under Scenario L from the normal approximation.
#'     Equals `alpha_L` by construction and is not the attained probability.
#'   - PCS_H: Nominal PCS under Scenario H from the normal approximation.
#'     Equals `alpha_H` by construction and is not the attained probability.
#'   - PCS_L_exact, PCS_H_exact: PCS the design actually attains at this
#'     `(n, lambda_u)`, by exact multinomial enumeration. At the small sample
#'     sizes typical of dose optimization studies these can fall materially
#'     below the target; use [calc_sample_size_utility_exact()] when they do.
...
calc_sample_size_utility_approx <- function(
  ...
  # ===== Nominal PCS (normal approximation) =====
  # These equal the targets alpha_L / alpha_H by construction, because n and
  # lambda_u were solved from these same normal equations. They are NOT the
  # probabilities the design attains -- see PCS_L_exact / PCS_H_exact below.
  pcs_L <- pnorm((lambda_u - delta_mu_L) / sqrt(v_L / n))
  pcs_H <- 1 - pnorm((lambda_u - delta_mu_H) / sqrt(v_H / n))
  
  # ===== Attained PCS (exact multinomial enumeration) =====
  attained <- calc_attained_pcs(pi_L_L, pi_H_L, pi_L_H, pi_H_H,
                                u = u, n = n, lambda_u = lambda_u, den = den)
  ...
  result <- list(
    ...
    PCS_L = pcs_L,
    PCS_H = pcs_H,
    PCS_L_exact = attained$PCS_L,
    PCS_H_exact = attained$PCS_H,
    scenario_L = list(
      delta_mu = delta_mu_L, 
      v = v_L,
      pL = pL_L, 
