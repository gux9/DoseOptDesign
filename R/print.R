...
print.rose_design <- function(x, ...) {
  ...
  # Attained vs nominal PCS.  For the approximate method the nominal values are
  # the normal-approximation probabilities and equal the targets by
  # construction; the attained values come from exact enumeration.
  has_exact <- !is.null(x$PCS_L_exact) && !is.null(x$PCS_H_exact) &&
    is.finite(x$PCS_L_exact) && is.finite(x$PCS_H_exact)
  if (has_exact) {
    cat("PCS (nominal, normal approximation):\n")
    cat(sprintf("  PCS_L: %.4f (target: %.2f)\n", x$PCS_L, x$inputs$alpha_L))
    cat(sprintf("  PCS_H: %.4f (target: %.2f)\n\n", x$PCS_H, x$inputs$alpha_H))
    cat("PCS attained (exact):\n")
    cat(sprintf("  PCS_L: %.4f (target: %.2f)\n", x$PCS_L_exact, x$inputs$alpha_L))
    cat(sprintf("  PCS_H: %.4f (target: %.2f)\n", x$PCS_H_exact, x$inputs$alpha_H))
    short <- c(x$PCS_L_exact - x$inputs$alpha_L, x$PCS_H_exact - x$inputs$alpha_H)
    if (min(short) < -1e-4) {
      cat(sprintf(paste0("  NOTE: the attained PCS falls %.3f below target. The ",
                         "normal approximation\n        is optimistic at this ",
                         "sample size; use calc_sample_size_%s_exact()\n",
                         "        for a design that meets the target.\n"),
                  -min(short), if (inherits(x, "rose_design")) "rose" else "utility"))
    }
    cat("\n")
  } else {
    cat("Achieved PCS:\n")
...
print.dose_design <- function(x, ...) {
  ...
  # The scenario lists store the variance as `v`; `variance` is the ROSE
  # spelling. Reading only `variance` silently dropped the line below.
  scen_v <- function(s) if (!is.null(s$v)) s$v else s$variance
  ...
  cat(sprintf("  Delta_mu=%.4f, v=%.4f\n\n", 
              x$scenario_L$delta_mu, scen_v(x$scenario_L)))
  ...
  cat(sprintf("  Delta_mu=%.4f, v=%.4f\n\n", 
              x$scenario_H$delta_mu, scen_v(x$scenario_H)))
